...
import { useQualityCheck } from "@/app/(protected)/(main-pages)/quality-check/hooks/use-quality-check";
import { useProfile } from "@/hooks/use-profile";
import { updateQualityTestAction } from "../../server-actions";
import type { QualityTest } from "@/types";

// ── Status chip ──────────────────────────────────────────────────────────────

type CaptureStatus = "captured" | "to-capture";
      ...
      >
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {status === "captured" && capturedLabel && (
            <span className="text-xs text-gray-500 font-medium">{capturedLabel}</span>
          )}
        </div>
      </button>
      {expanded && (
...
export function EditTestForm({ test }: { test: QualityTest }) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { profile } = useProfile();
  const isAdmin = profile?.role === "ADMIN";

  const { deleteTest, isDeleting } = useQualityCheck();
  ...
  const form = useForm<CreateQualityTestFormValues>({
    ...
    defaultValues: {
      ...
      production_line: test.production_line ?? "",
      color_l: test.color_l ?? undefined,
      color_a: test.color_a ?? undefined,
      color_b: test.color_b ?? undefined,
      color_image_url: test.color_image_url ?? undefined,
      texture_brix: test.texture_brix ?? undefined,
      viscosity_cp: test.viscosity_cp ?? undefined,
      taste_flavour_score: test.taste_flavour_score ?? undefined,
      cooking_color_score: test.cooking_color_score ?? undefined,
      cooking_color_image_url: test.cooking_color_image_url ?? undefined,
      cooking_taste_score: test.cooking_taste_score ?? undefined,
      cooking_notes: test.cooking_notes ?? "",
    },
  });
  ...
  async function handleSubmit(
    ...
    setLoading(true);
    try {
      const { error } = await updateQualityTestAction(test.id, values, status);
      if (error) {
        toast.error(error);
        return;
      }
      toast.success(status === "pending" ? "Batch report updated." : "Saved as draft.");
      queryClient.invalidateQueries({ queryKey: QC_KEYS.all });
      queryClient.invalidateQueries({ queryKey: QC_KEYS.detail(test.id) });
  ...
  async function handleCameraCapture(file: File, labValues?: LabValues, textureScore?: number | null) {
    ...
    try {
      ...
      const { publicUrl, error } = await uploadImageToStorage(file, test.id, batchId);
      if (error) {
      ...
      <Form {...form}>
        <div className="space-y-4 pb-40">
          ...
          <div className="space-y-2">
            ...
            <ParameterCard
              ...
              capturedLabel={watchTexture != null ? `${watchTexture} °Bx` : undefined}
            >
              <FormField
                control={form.control}
                name="texture_brix"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs text-muted-foreground">Brix Value (°Bx)</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.01"
            ...
            <ParameterCard
              ...
              capturedLabel={watchViscosity != null ? `${watchViscosity} cP` : undefined}
            >
              <FormField
                control={form.control}
                name="viscosity_cp"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs text-muted-foreground">Viscosity (cP)</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.1"
            ...
            <ParameterCard
              ...
              capturedLabel={watchTaste != null ? `Score: ${watchTaste}` : undefined}
            >
              <FormField
                control={form.control}
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs text-muted-foreground">Score (0–10)</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.1"
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-sm text-gray-900">Cooking Performance</span>
              </div>
              {cookingExpanded ? (
          ...
          {isAdmin && (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 min-h-[48px] rounded-2xl border border-destructive/30 text-destructive text-sm font-semibold hover:bg-destructive/5 transition-colors"
            >
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-gray-200 p-4 space-y-2 md:hidden">
          <Button
            ...
            onClick={form.handleSubmit((values) => handleSubmit(values, "pending"))}
          >
            {isSubmitting ? "Updating..." : "Submit Batch Report"}
          </Button>
          <Button
            type="button"
            variant="ghost"
